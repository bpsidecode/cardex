...
void main() {
  ...
  testWidgets('Discover sorts by each option and keeps search and tab state', (tester) async {
    await tester.pumpWidget(MaterialApp(home: HomeScreen(service: service)));

    String firstBrand() => tester.widget<BrandTile>(find.byType(BrandTile).first).brand.name;
    Future<void> chooseSort(String label) async {
      await tester.tap(find.byTooltip('Sort brands'));
      await tester.pumpAndSettle();
      await tester.tap(find.ancestor(
        of: find.text(label),
        matching: find.byWidgetPredicate((widget) => widget is CheckedPopupMenuItem),
      ));
      await tester.pumpAndSettle();
    }
    ...
    expect(tester.widgetList<BrandTile>(find.byType(BrandTile)).take(2).map((tile) => tile.brand.name), ['BYD', 'Chery']);
  ...
  testWidgets('Explore categories expand, collapse, and survive tab changes', (tester) async {
    await tester.pumpWidget(MaterialApp(home: HomeScreen(service: service)));
    await tester.tap(find.text('Explore'));
    await tester.pumpAndSettle();
    final countrySection = find.widgetWithText(ExpansionTile, 'Country collections');
    expect(find.descendant(of: countrySection, matching: find.byType(ListTile)), findsOneWidget);
    await tester.tap(find.text('Country collections'));
    await tester.pumpAndSettle();
    expect(find.descendant(of: countrySection, matching: find.byType(ListTile)).evaluate().length, greaterThan(1));
    await tester.tap(find.text('Discover').last);
    await tester.pumpAndSettle();
